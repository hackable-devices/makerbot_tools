//Original GenHoughTrnf can be found at http://www.business-to-technology.com/
//Insert copyright here: gpl ? mit ?
 ...
 public:
     ...
     GenHoughTrnf(string templateimage, string countourimage){
         // default values
         templatepath = templateimage;
         contourpath = countourimage;
     ...
     void createTemplate(Mat frame){
         imwrite("files/template_star.bmp", frame);
         Mat detected_edges;
         blur( frame, detected_edges, Size(3,3) );
         cv:Mat thres_img;
         double high_thres = cv::threshold( frame, thres_img, 0, 255, CV_THRESH_BINARY+CV_THRESH_OTSU ); //thresh detection
         cout<<"plopenzie1"<<endl;
         Canny( detected_edges, detected_edges, high_thres*0.5, high_thres, 3 );
         cout<<"plopenzie2"<<endl;
 ...
 private:
     ...
     void readPoints(){
         ...
         Mat original_img = imread(templatepath, 1);
         ...
         Mat template_img = imread(contourpath, 1);
         ...
         for (int j=0; j<nl; ++j) {
             ...
             for (int i=0; i<nc; ++i) {              
                 //if ( data[i]==Vec3b(127,127,127)  ){} //old code
                 //getting a pure gray pixel is hard with lots of editors. So we will take into account any pixel that is not black nor white as the reference point
                 if ( data[i]!=Vec3b(0,0,0) && data[i]!=Vec3b(255,255,255) ){
                     refPoint = Vec2i(i,j);
                 }
             }
         }
 ...
 { 
     //Type in the terminal (v4l has to be installed): uv4l --driver raspicam --auto-video_nr --width 1280 --height 720 --encoding yuv420 --bitrate 30000000
     
     cout << "Print Supervisor V0.1 is starting..." << endl;
     ...
     int minx=0, maxx=1200, miny=0, maxy=700, cx=670, cy=170, lx=150, ly=150; //area in which to look for the artefact
     string templatepath = "files/template_star.bmp", contourpath = "files/contour_def01.bmp", backgroundpath = "files/decoy.bmp"; //default
     ...
     if (fs.isOpened())
     {
            ...
            fs["miny"] >> miny;
            fs["maxy"] >> maxy;
            fs["templatepath"] >> templatepath;
            fs["contourpath"] >> contourpath;
            fs["backgroundpath"] >> backgroundpath;
            fs["cx"] >> cx;
            fs["lx"] >> lx;
            fs["cy"] >> cy;
            fs["ly"] >> ly;
     ...
     while (com != "stop")
     {
         ...
         if( (command != "printpreview")&&(command != "maketemplate")&&(command != "stopapcheck")  )
         {
             ...
             modified = imread(backgroundpath,0); //size of the black screen // TODO filename ?
             //cout << "[DEBUG] 3.1" << endl; //test
             cv::Rect rio0(cx, cy, lx, ly); //position of the area to crop
             cv::Mat crop = gray(rio0);
             //cout << "[DEBUG] 3.2" << endl; //test
             
             cv::Rect roi( cv::Point( 30, 30 ), crop.size() ); //position at the black screen
             //cout << "[DEBUG] 3.3: " << cx << " " << cy << " " << lx << " " << ly << endl; //test
             Mat temp = modified( roi );
             //cout << "[DEBUG] 3.4" << endl; //test
         ...
         else if(command == "stopapcheck")
         {
             ...
             nonairprint = 0;
             airprint = 0;
             //ght.createTemplate(modified); //only for testing and quick templete generation (since I didnt want to make a new button for an inexisting / mostly manual feature)
         ...
         else if(command == "configureartefact") //TODO needs testing
         {
             ...
             fs2 << "COMMENT" << "How to use this file: minx, maxx, miny, maxy are the bounding box where the target is. Generated by the applcation with a setup and should thus not be manually set. templatepath, contourpath, backgroundpath are paths to the template image as well as it's contour and the background image. cx and cy are the coordinates of the starting point of the binding rectangle of size lx times ly."; 
             fs2 << "minx" << minx;
             fs2 << "maxx" << maxx;
             fs2 << "miny" << miny;
             fs2 << "maxy" << maxy;
             fs2 << "templatepath" << templatepath;
             fs2 << "contourpath" << contourpath;
             fs2 << "backgroundpath" << backgroundpath;
             fs2 << "cx" << cx;
             fs2 << "lx" << lx;
             fs2 << "cy" << cy;
             fs2 << "ly" << ly;
